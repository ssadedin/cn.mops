...
\section{cn.MOPS for haploid genomes}
For haploid genomes the prior assumption is that all samples have copy number 1.
The function {\tt haplocn.mops} implements the cn.MOPS algorithm adjusted to
haploid genomes. 
...
For detection of CNVs in a setting where the normal state is known, the
function {\tt referencecn.mops} can be applied. It implements the cn.MOPS 
algorithm adjusted to this setting. For tumor samples very high copy numbers 
can be present -- the maximum copy number with the default setting is 8 -- and
cn.mops has to be adjusted to allow higher copy numbers.
